...
public class FloatCenterLeftView extends LinearLayout implements
	...
	private void initView(Context ctx) {
		...
		lp_lleft.rightMargin = MetricUtil.getDip(ctx, 45);
		...
		txt_quetion.setPadding(MetricUtil.getDip(ctx, 10),
				MetricUtil.getDip(ctx, 5), MetricUtil.getDip(ctx, 5),
				MetricUtil.getDip(ctx, 5));
		...
		txt_forum.setPadding(MetricUtil.getDip(ctx, 6),
				MetricUtil.getDip(ctx, 5), MetricUtil.getDip(ctx, 5),
				MetricUtil.getDip(ctx, 5));
	...
	private void updatePostion() {
		FloatWindowService.disPlayCenterView(getContext(), 1);
	}
