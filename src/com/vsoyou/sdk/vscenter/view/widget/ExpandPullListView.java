...
public class ExpandPullListView extends ExpandableListView implements OnScrollListener {
	...
	public ExpandPullListView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();
			mFooterView.setStatus(ExpandFooterView.STATE_NORMAL);
			// make sure "pull up" don't show a line in bottom when listview
			// with one page
			setFooterDividersEnabled(false);
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setSate(ExpandHeaderView.STATE_READY);
			} else {
				mHeaderView.setSate(ExpandHeaderView.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setStatus(ExpandFooterView.STATE_READY);
			} else {
				mFooterView.setStatus(ExpandFooterView.STATE_NORMAL);
			}
		}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setStatus(ExpandFooterView.STATE_LOADING);
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh
					...
					mHeaderView.setSate(ExpandHeaderView.STATE_REFRESHING);
					if (mListViewListener != null) {
